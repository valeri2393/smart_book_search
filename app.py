...
books = pd.read_csv('data/data_cleaned.csv')
...
if text and recommend_button:  # Check if the user entered text and clicked the button
    ...
    for i in indices[0]:
        ...
        with col1:
            st.image(books['image_url'][i], use_column_width=True)
        
        with col2:
            st.write(f"**Название книги:** {books['title'][i]}")
            st.write(f"**Автор:** {books['author'][i]}")
            st.write(f"**Описание:** {books['annotation'][i]}")
